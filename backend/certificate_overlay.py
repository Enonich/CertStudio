...
def register_fonts_from_directory(fonts_dir: Path) -> dict[str, str]:
    """Auto-register supported TTF fonts from a directory.
    ...
    # ReportLab TTFont does not support many OTF/CFF fonts; explicitly skip them.
    for font_file in fonts_dir.glob("*.otf"):
        print(f"[SKIP] Ignoring unsupported OTF font (use .ttf): {font_file.name}")
...
def draw_overlay(
    ...
    fields_list = list(fields_cfg.get("fields", []))
    images_list = list(fields_cfg.get("images", []))
    draw_items = [
        *({"kind": "field", "z": int(field.get("z", index)), "payload": field} for index, field in enumerate(fields_list)),
        *({"kind": "image", "z": int(image.get("z", len(fields_list) + index)), "payload": image} for index, image in enumerate(images_list)),
    ]
    draw_items.sort(key=lambda item: item["z"])

    image_cache: dict[str, ImageReader | None] = {}

    for draw_item in draw_items:
        if draw_item["kind"] == "field":
            field = draw_item["payload"]
            name = field.get("name")
            ...
            wrap_width = field.get("wrap_width")
            has_newlines = "\n" in text
...
def main() -> None:
    ...
    # the script's own fonts/ sibling (legacy fallback), and the canonical
    # assets/fonts/ directory at the project root.
    script_dir = Path(__file__).parent
    fonts_dirs = [
        fields_path.parent / "fonts",
        script_dir / "fonts",
